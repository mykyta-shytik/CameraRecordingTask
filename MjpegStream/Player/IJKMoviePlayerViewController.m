...
@property (nonatomic, strong) RKRecorder *rec;
...
//    [[UIApplication sharedApplication] setStatusBarOrientation:UIInterfaceOrientationLandscapeLeft animated:NO];
    ...
    [self.mediaControl autoPinEdgesToSuperviewEdges];
...
{
    if (self.isRecording) {
        [self.rec stop:self completion:^(NSError *error) {
            NSLog(@"%@", error);
        }];
        self.recording = NO;
    } else {
        [self.rec start:@"video5" completion:^(NSError *error) {
            NSLog(@"%@", error);
        }];
        self.recording = YES;
    }
